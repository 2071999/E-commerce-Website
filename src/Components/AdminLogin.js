...
export default function AdminLogin() {
    ...
    const checkSession = async()=>{
        ...
        const x = await getItemByFilter("Admin", ['Email', '==', detail.Email]);
        let isValid = false;
    ...
    const login = async ()=>{
        setFlag(true)
        const x = await getItemByFilter("Admin", ['Email', '==', formData.Email]);
        let isValid = false;
        let item = {};
        x.forEach(element=>{
        ...
        if(isValid){
            localStorage.setItem('adminStamp', JSON.stringify(item));
            window.location.href="/admin"
        }
        else{
